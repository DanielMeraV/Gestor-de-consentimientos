...
const crearPersona = async (req, res) => {
    try {
        const { Nombre, Apellido, Identificacion, FechaNacimiento, Telefono, Correo, Direccion, TipoUsuario } = req.body;

        if (!Nombre || !Apellido || !Identificacion || !FechaNacimiento || !Telefono || !Correo || !Direccion) {
        ...
        await sequelize.query(query, {
            replacements: {
                ...
                Correo,
                Direccion,
                TipoUsuario: TipoUsuario || 'cliente', // Valor por defecto
            },
...
const obtenerPersonas = async (req, res) => {
    try {
        ...
        const personas = await Persona.findAll({
            attributes: [
                'PersonaID',
                [sequelize.literal(`CAST(DecryptByKey(Nombre) AS NVARCHAR(100))`), 'Nombre'],
                [sequelize.literal(`CAST(DecryptByKey(Apellido) AS NVARCHAR(100))`), 'Apellido'],
                [sequelize.literal(`CAST(DecryptByKey(Identificacion) AS NVARCHAR(20))`), 'Identificacion'],
                [sequelize.literal(`CAST(DecryptByKey(FechaNacimiento) AS NVARCHAR(10))`), 'FechaNacimiento'],
                [sequelize.literal(`CAST(DecryptByKey(Telefono) AS NVARCHAR(15))`), 'Telefono'],
                [sequelize.literal(`CAST(DecryptByKey(Correo) AS NVARCHAR(100))`), 'Correo'],
                [sequelize.literal(`CAST(DecryptByKey(Direccion) AS NVARCHAR(255))`), 'Direccion'],
                'TipoUsuario'
            ]
        });
